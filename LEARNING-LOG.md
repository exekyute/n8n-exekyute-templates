...
## 2026-08-18, at 55 published

A third submission bounced this week, for the opposite fault: not too basic, too familiar. The reviewer called it very similar to my other templates with only a few nodes swapped out, and declined to publish a near-duplicate. That is the rule from two entries ago enforced from the outside: the library treats a creator's catalog as one body of work, and a workflow that reuses a shipped mechanism with different nouns reads as a re-skin no matter how sound the build is. The two failure classes bracket the same idea. Too basic is a workflow with no mechanism. Too similar is a mechanism I already shipped. What publishes is a mechanism this catalog does not already carry. So the pre-submission diff now runs in both directions: against the library for originality, and against my own published list for overlap, the same referee I already run on idea batches. When a variant is genuinely worth keeping, the reviewer's own suggestion is the right shape: fold it into the existing template as an optional branch with a customization note, one entry with variation inside it rather than two entries a few nodes apart.

## 2026-08-17, at 54 published

Two submissions came back this week as too basic to publish. The census taught me the ceiling: winners are barely bigger than the dead middle, so size buys nothing. It could not teach me the floor, because rejected workflows never enter the corpus. Survivorship, doing what survivorship does. Rereading the two builds with a reviewer's eyes, the problem is plain: each one is a route, not a mechanism. Read a table, apply one rule, send the messages. Clean stickies and tidy names do not change what the canvas is. The floor, stated as a test I can run before building anything: describe the workflow in one sentence without naming the services. If that sentence contains a decision that changes behavior, state that carries between runs, or an error path that does more than stop, it is a template. If it reads like fetch X and notify Y, it is a snippet, and it either grows a gate, a ledger, or a referee, or it stays home. Yesterday's entry said one mechanism ships once. This week adds the corollary: no mechanism, no ship.
